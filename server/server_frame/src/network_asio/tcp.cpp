...
namespace moon
{
    tcp::tcp(asio::io_context& ctx) noexcept
        :type_(PTYPE_SOCKET)
    ...
    void tcp::set_enable_frame(std::string flag)
    {
        ...
        default:
            LOG_WARN("tcp::set_enable_frame Unsupported  enable frame flag {}.Support: 'r' 'w' 'wr' 'rw'.", flag.data());
            break;
        }
    }
    ...
    bool tcp::listen(const std::string & ip, const std::string & port)
    {
        ...
        {
            LOG_ERROR("{}:{} {}({})", ip, port, e.what(), e.code().value());
            return false;
        }
    }
    ...
    void tcp::async_accept(int32_t responseid)
    {
        ...
        acceptor_->async_accept(connection->socket(), [this, self = shared_from_this(), connection, responseid](const asio::error_code& e)
        {
            ...
            else
            {
                ...
                {
                    LOG_WARN("tcp async_accept error {}({})", e.message().data(), e.value());
                }
            }
        });
    }
    ...
    uint32_t tcp::connect(const std::string & ip, const std::string & port)
    {
        ...
        {
            LOG_WARN("{}:{} {}({})", ip.data(), port.data(), e.what(), e.code().value());
            return 0;
        }
    }
    ...
    bool tcp::init(std::string_view config)
    {
