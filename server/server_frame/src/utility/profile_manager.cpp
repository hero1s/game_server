...
ProfileNode *ProfileNode::getSubNode(const char *node_name) {
    ...
    LOG_DEBUG("new node:{} add anazy",node_name);
    return new_node;
}
...
bool ProfileManager::saveData(const char *servername, uint32_t serverid, uint32_t tid) {
    ...
    sprintf(data_path, "%s%d%02d%02d_%02d%02d%02d_%s_%u_%u.log", app_path, lt->tm_year + 1900, lt->tm_mon + 1,
            lt->tm_mday, lt->tm_hour, lt->tm_min, lt->tm_sec, servername, serverid, tid);
