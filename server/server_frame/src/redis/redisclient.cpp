...
namespace {
    ...
    ssize_t socketWriteImpl(int socket, const char* buffer, size_t size,size_t timeoutMsec)
    {
        struct timeval tv = {static_cast<time_t>(timeoutMsec/1000),
                             static_cast<__suseconds_t>((timeoutMsec%1000)*1000)};
        int result = setsockopt(socket, SOL_SOCKET, SO_RCVTIMEO, &tv, sizeof(tv));
    ...
    size_t socketWrite(int socket, asio::const_buffer buffer,const uint32_t& timeout,asio::error_code& ec)
    {
        size_t bytesSend = 0;
        size_t timeoutMsec = timeout;
    ...
    size_t socketWrite(int socket, const std::vector<asio::const_buffer>& buffers,const uint32_t& timeout,asio::error_code& ec)
    {
        size_t bytesSend = 0;
...
namespace redisclient {
    ...
    void RedisSyncClient::connect(const asio::ip::tcp::endpoint& endpoint, asio::error_code& ec)
    {
        ...
        if (result<0) {
            if (errno==EINPROGRESS) {
                for (;;) {
                    ...
                    result = ::poll(&pfd, 1, connectTimeout);
    ...
    void RedisSyncClient::installErrorHandler(std::function<void(const std::string&)> handler)
    {
        pimpl->errorHandler = std::move(handler);
    }
    ...
    RedisSyncClient& RedisSyncClient::setConnectTimeout(const uint32_t& timeout)
    {
        connectTimeout = timeout;
        return *this;
    }

    RedisSyncClient& RedisSyncClient::setCommandTimeout(const uint32_t& timeout)
    {
...
namespace redisclient {
    ...
    {
        assert(isArray());
        return std::get<std::vector<RedisValue>>(value);
    }
