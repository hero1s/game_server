...
void CStringUtility::ToLower(char* szSrc)
{
	...
	for (int i = 0; szSrc[i] != '\0'; i++) szSrc[i] = tolower(szSrc[i]);
}
...
void CStringUtility::ToLower(char* szSrc, size_t iLen)
{
	...
	for (size_t i = 0; i < iLen; i++) szSrc[i] = tolower(szSrc[i]);
}
...
std::string& CStringUtility::GBKToLower(std::string& strSrc)
{
	...
	{
		if (strSrc[i] < 0) i++;
		else strSrc[i] = tolower(strSrc[i]);
	}
...
void CStringUtility::ToUpper(char* szSrc)
{
	...
	for (int i = 0; szSrc[i] != '\0'; i++) szSrc[i] = toupper(szSrc[i]);
}
...
void CStringUtility::ToUpper(char* szSrc, size_t nLen)
{
	...
	for (size_t i = 0; i < nLen; i++) szSrc[i] = tolower(szSrc[i]);
}
...
std::string& CStringUtility::GBKToUpper(std::string& strSrc)
{
	...
	{
		if (strSrc[i] < 0) i++;
		else strSrc[i] = toupper(strSrc[i]);
	}
...
bool CStringUtility::StartWith(std::string& strSrc, const std::string& strStart)
{
	...
	if (strSrc.size() >= nLen)
	{
		...
		{
			if (strSrc[i] != strStart[i]) return false;
		}
		return true;
	}
	else return false;
...
bool CStringUtility::StartWithNoCase(std::string& strSrc, const std::string& strStart)
{
	...
	if (strSrc.length() >= nLen)
	{
		for (size_t i = 0; i < nLen; i++)
		{
			if (tolower(strSrc[i]) != tolower(strStart[i])) return false;
		}
		return true;
	}
	else return false;
...
bool CStringUtility::EndWith(std::string& strSrc, std::string& strEnd)
{
	...
	if (nSrcLen >= nLen)
	{
		for (size_t i = 0; i < nLen; i++)
		{
			if (strSrc[nSrcLen - nLen + i] != strEnd[i]) return false;
		}
...
bool CStringUtility::EndWithNoCase(std::string& strSrc, std::string& strEnd)
{
	...
	if (nSrcLen >= nLen)
	{
		for (size_t i = 0; i < nLen; i++)
		{
			if (tolower(strSrc[nSrcLen - nLen + i]) != tolower(strEnd[i])) return false;
		}
		return true;
	}
	else return false;
...
{
	size_t iLen = 0;
	if (strFromSet.size() > strToSet.size()) iLen = strFromSet.size();
	else iLen = strToSet.size();
	...
	for (size_t i = 0; i < strSrc.size(); i++)
	{
		...
		if (dwPos == std::string::npos) strResult += ch;
		else if (dwPos < strToSet.size()) strResult += strToSet[dwPos];
	}
...
CStringUtility::FindNoCase(const std::string& strSrc, const std::string& strSub, size_t dwFrom)
{
	...
	if (it == strSrc.end()) return std::string::npos;
	else return (it - strSrc.begin());
}
...
{
	if (NULL == pcSrc || NULL == pcSub) return NULL;

	if (!CheckGbKString(pcSrc) || !CheckGbKString(pcSub)) return NULL;

	int nLen    = strlen(pcSrc);
	int nSubLen = strlen(pcSub);
	if (nLen < nSubLen) return NULL;

	for (int i = 0; i < nLen; i++)
	{
		if (i + nSubLen > nLen) return NULL;
...
{
	if (!CheckGbKString(strSrc) || !CheckGbKString(strSub)) return std::string::npos;

	std::string::size_type nLen    = strSrc.size();
	std::string::size_type nSubLen = strSub.size();

	if (nLen < nSubLen) return std::string::npos;
	if (nLen == 0 || nSubLen == 0) return std::string::npos;

	for (std::string::size_type i = 0; i < nLen; i++)
	{
		if (i + nSubLen > nLen) return std::string::npos;
...
bool CStringUtility::IsSubStrContained(const std::string& strSrc, const std::vector<std::string>& sSubStr)
{
	...
	{
		if (std::string::npos != strSrc.find(*it)) return true;
	}
...
bool CStringUtility::IsSubStrAllContained(const std::string& strSrc, const std::vector<std::string>& sSubStr)
{
	...
	for (it = sSubStr.begin(); it != sSubStr.end(); it++)
	{
		if (std::string::npos == strSrc.find(*it)) return false;
	}
...
void CStringUtility::StatStr(const std::string& strWord, size_t& dwAlpha, size_t& dwDigit)
{
	...
	for (size_t i = 0; i < strWord.size(); ++i)
	{
		if ((strWord[i] >= 'a' && strWord[i] <= 'z') || (strWord[i] >= 'A' && strWord[i] <= 'Z'))
			dwAlpha++;
		else if (strWord[i] >= '0' && strWord[i] <= '9') dwDigit++;
	}
}
...
size_t CStringUtility::GetSubStrCnt(const std::string& strSrc, const std::string strSubStr, bool bNoCase)
{
	...
	do
	{
		if (bNoCase) dwPos = CStringUtility::FindNoCase(strSrc, strSubStr, dwPos + strSubStr.size());
		else dwPos = strSrc.find(strSubStr, dwPos + strSubStr.size());
		if (std::string::npos != dwPos) dwSubStrCnt++;
		else break;
	} while (true);
...
bool CStringUtility::IsEnglishString(const std::string& strSrc, bool bLowLineAsAlpha)
{
	for (size_t i = 0; i < strSrc.size(); i++)
	{
		if (bLowLineAsAlpha)
		{
			if (isalpha(strSrc[i]) == 0 && '_' != strSrc[i]) return false;
		}
		else
		{
			if (isalpha(strSrc[i]) == 0) return false;
		}
	}
	return true;
}
...
bool CStringUtility::IsDigitString(const std::string& strSrc)
{
	for (size_t i = 0; i < strSrc.size(); i++)
	{
		if (isdigit(strSrc[i]) == 0) return false;
	}
	return true;
}
...
bool CStringUtility::IsOnlyComposedOfGB2312Cha(const std::string& strSrc)
{
	...
	if (strSrc.size()%2 != 0) return false;
	...
	for (size_t i = 0; i + 1 < strSrc.size(); i = i + 2)
	{
		...
		if (iIndex < 0 || iIndex > 6767) return false;
	}
	return true;
}
...
bool CStringUtility::IsSeparator(char c)
{
	...
	if (std::string::npos == strSeparator.find(c)) return false;
	else return true;
}
...
bool CStringUtility::IsDBCS(const unsigned char szStr[2])
{
	return ((szStr[0] > 0xa0 && szStr[1] > 0xa0)                                //HZ_GB
			|| (szStr[0] > 0xa0 &&
					((szStr[1] > 0x3f && szStr[1] < 0x7f) || (szStr[1] > 0xa0)))                //HZ_BIG5
			|| (((szStr[0] > 0x80 && szStr[0] < 0xa0) || (szStr[0] > 0xdf && szStr[0] < 0xfd)) &&
					(szStr[1] > 0x3f && szStr[1] < 0xfd))
					//HZ_JPN
			|| (szStr[0] > 0x80 &&
					((szStr[1] > 0x3f && szStr[1] < 0x7f)
							|| (szStr[1] > 0x7f && szStr[1] < 0xff)))        //HZ_GBK
			|| ((szStr[0] > 0xaf && szStr[0] < 0xf8) && szStr[1] > 0xa0)                            //HZ_TGB
			|| (((szStr[0] == 0xa1) || (szStr[0] == 0xa3))
					&& szStr[1] > 0xa0)                        //HZ_TSIGN
			|| (((szStr[0] == 0xa4) || (szStr[0] == 0xa5)) &&
					(szStr[1] > 0xa0 && szStr[1] < 0xf4)));            //HZ_TJPN
}
...
bool CStringUtility::CheckGbKString(const std::string& strSrc, bool bContainsNprint)
{
	...
	for (size_t i = 0; i < nLen; i++)
	{
		...
		else if (!bContainsNprint && !isprint(strSrc[i])) return false;
	}
...
std::string CStringUtility::DecodeUrlStr(const std::string& strSrc)
{
	...
	for (size_t i = 0; i < strSrc.size(); i++)
	{
		...
		else strDes.push_back(strSrc[i]);
	}
	return strDes;
}
                                             ...
                                             size_t iFromIndex, bool bNoCase)
{
	...
	if (iPrefixIndex == std::string::npos) return false;
	...
	if (iSuffixIndex == std::string::npos)return false;
...
void CStringUtility::clear_html_tag(std::string& strSrc)
{
	...
	for (size_t i         = 0; i < strSrc.size(); i++)
	{
		...
		{
		case STATUS_TEXT:
			if (c == '<') flag = STATUS_IN_HTML;
			else strResult.push_back(c);
			break;

		case STATUS_IN_HTML:
			if (c == '>') flag = STATUS_TEXT;
			break;
		default:
...
{
	if (h >= 0 && h <= 9) return static_cast<char>(h + '0');
	else if (h >= 10 && h <= 15) return static_cast<char>(h - 10 + 'a');
	else return '-';
}
...
{
	if (dSize >= 100) sprintf(s_size, "%3.0f%s", dSize, s);
	else if (dSize >= 10) sprintf(s_size, "%3.1f%s", dSize, s);
	else sprintf(s_size, "%3.2f%s", dSize, s);
}
