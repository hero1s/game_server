...
class ProfileNode
{
    public:
        ...
        //@brief    得取当前线程ID
        uint32 getCurThreadID() const {return m_cur_threadid;};
        //@brief    设置当前线程ID
        void setCurThreadID(uint32 threadid) {m_cur_threadid = threadid;};
        //@brief    保存统计数据
        bool saveData(void* file, uint32 layer/*= 0*/);
        //@brief    指定统计信息
        void statInfo(uint32 flag);

    private:
        uint32 m_cur_threadid;          //当前线程ID
        uint32 m_total_calls;           //总调用次数
        int m_recursion_counter;        //递归调用计数
        float m_total_time;             //总时间
        float m_peak_value;             //此节点执行时间峰值
        float m_percent_in_parent;      //占用父节点的时间百分比
        int64 m_start_time;             //开始时间
...
class ProfileManager
{
    public:
        ...
        //@brief    设置统计信息
        void statInfo(uint32 flag /*= flag_stat_percentinparent */);
        //@brief    重置
        void reset(void);
        //@brief    增加帧数，应该在第帧中调用
        void increaseFrameCount(void){++m_frame_counter;};
        //@brief    取得经过的帧数
        uint32 getFrameCount(void){return m_frame_counter;};
        //@brief    取得根节点
        ProfileNode* getRootNode(void){return m_root_node;};
        //@brief    设置性能统计类型
        void setProfileType(uint8 profile_type){m_profile_type = profile_type;};
        //@brief    得到性能统计类型
        uint8 getProfileType(void){return m_profile_type;};
        //@brief    得到重置以来的时间
        float getTimeSinceReset();
        //@brief    保存数据到指定文件
        bool saveData(const char* file_name);
        //@brief    保存数据到默认文件
        bool saveData(const char* servername, uint32 serverid, uint32 tid = 0);
    private:
        ProfileNode* m_root_node;            //根节点
        ProfileNode* m_cur_node;             //当前节点
        uint8 m_profile_type;                //性能统计类型
        uint32 m_frame_counter;              //帧数
        int64 m_reset_time;                  //开始计时时间
};
...
{
public:
    typedef std::map<uint32, ProfileManager*>       MGR_MAP;
    typedef MGR_MAP::iterator                       ITER_MGR_MAP;

    GlobalProfileManager();
    ~GlobalProfileManager();
    ProfileManager* getMgr();
    void shutdown(const char* servername, uint32 serverid);
