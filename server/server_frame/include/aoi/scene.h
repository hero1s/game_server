...
namespace aoi {
    ...
    public:
        ...
        void insert(math::objectid_t uid, float x, float y) {
            ...
            scene_objs.try_emplace(uid, CSceneObj());
            is_need_check = true;
            check_aoi();
        }
    ...
    protected:
        void compare_view(math::objectid_t uid, CSceneObj &p, const std::vector<math::objectid_t> &query_result) {
            p.version += 2;
        ...
        void process_view_event(math::objectid_t uid, CSceneObj &p) {
            enter_list.clear();
            leave_list.clear();
            for (auto &v : p.view) {
