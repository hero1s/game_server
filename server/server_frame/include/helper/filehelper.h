...
namespace svrlib {
    ...
    public:
        ...
        static uint64_t GetFileSize(const std::string &pFileName) {
            struct stat64 buf;
        ...
        static bool Rename(const std::string &pOldName, const std::string &pNewName) {
            return 0 == rename(pOldName.c_str(), pNewName.c_str());
        }

        /** 功能：取得文件最后访问时间 */
        static bool GetFileAccessTimeStr(const std::string &strFileName, std::string &strTime);

        /** 功能：取得文件创建时间 */
        static bool GetFileCreateTimeStr(const std::string &strFileName, std::string &strTime);

        /** 功能：取得文件最后修改时间 */
        static bool GetFileLastModifyTimeStr(const std::string &strFileName, std::string &strTime);

        /** 功能：取得文件最后访问时间 */
        static bool GetFileAccessTime(const std::string &strFileName, time_t &stTime);

        /** 功能：取得文件创建时间 */
        static bool GetFileCreateTime(const std::string &strFileName, time_t &stTime);

        /** 功能：取得文件最后修改时间 */
        static bool GetFileLastModifyTime(const std::string &strFileName, time_t &stTime);

        /** 功能：从完整的文件路径名中取得文件名 */
        static std::string ExtractFileName(const std::string &strFilePath);

        /** 功能：从文件名中取得扩展名 */
        static bool ExtractFileExtName(const std::string &strFilePath, std::string &strFileExt);
