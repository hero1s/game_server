...
//--------------------------------------------------------------------------------------------
CGateSvrMgr::CGateSvrMgr()
{
    bind_handler(this, net::svr::S2CS_MSG_REGISTER, &CGateSvrMgr::handle_msg_register_svr);
}
...
void CGateSvrMgr::OnTimer()
{
    CServerClientMgr::OnTimer();
    UpdateServerList();//test
}

bool CGateSvrMgr::Init()
{
...
void CGateSvrMgr::ShutDown()
{

}

// 更新服务器列表给全部服务器
void CGateSvrMgr::UpdateServerList()
{
...
int CGateSvrMgr::OnRecvClientMsg()
{
    m_msgMinCount++;
    if (_head->route>0) {
...
//路由分发消息
int CGateSvrMgr::OnRouteDispMsg()
{
    switch (_head->route) {
...
//服务器注册
int CGateSvrMgr::handle_msg_register_svr()
{
